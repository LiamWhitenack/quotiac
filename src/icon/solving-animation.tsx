...
function getCircleTicks(
  radius: number,
  numberOfTicks: number,
  dashLength: number
) {
  const circumference = 2 * Math.PI * radius;
  const dashAndGap = circumference / numberOfTicks;
  const gapLength = dashAndGap - dashLength;
  const dashArray = `${dashLength} ${gapLength}`;
  const dashOffset = dashLength / 2; // centers the first dash at top
...
const SolvingLockAnimation: FC<LockSvgProps> = ({
  ...
  const letterOpacity = useAnimatedProps(() => {
    // For example, fade in "Q" starting at 0.1, fully visible at 0.2
    return {
      opacity: animation.value
    };
  });

  const svgOpacity = useAnimatedProps(() => {
    // Fade out the preceding SVG
    return {
  ...
  const animatedProps = useAnimatedProps(() => {
    const angle = animation.value * 360 * 3
    if (isMobile) {
      return {
        transform: [
  ...
  const [smallTickStrokeDasharray, smallTickStrokeDashoffset] = getCircleTicks(65, 32, 3);
  const [bigTickStrokeDasharray, bigTickStrokeDashoffset] = getCircleTicks(63, 8, 4);
  ...
  return (
    <Svg
      ...
      strokeWidth={4}
    >
      <AnimatedG transform="translate(0 40)">
        <AnimatedPath
        ...
        <AnimatedG animatedProps={animatedProps}>
          ...
          <Circle
            ...
            strokeDashoffset={bigTickStrokeDashoffset}
          />
          <Circle
            cx={100}
            cy={180}
            ...
            strokeDashoffset={smallTickStrokeDashoffset}
          />


          <AnimatedG fill={theme.background}>
            <AnimatedG
              animatedProps={svgOpacity}
            ...
            <AnimatedG
              animatedProps={letterOpacity}><Text
                x={100}
                y={136}
...
export default SolvingLockAnimation;
